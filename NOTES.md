...
使用者要表格。三種做法（Markdown＋格子視窗＋閱讀模式／輸入區裡直接顯示表格／
整個換富文本編輯器）裡選了第一種。**內文是純文字是整個 app 的前提**：標籤、
章節目錄、快速跳轉（照行號）、段首縮排、復原、同步合併、匯出匯入、iPad 鍵盤
那一串修正都靠它。換成富文本等於全部重做。

- 表格存成 Markdown（`js/doc-table.js`），格子裡的 `|` 跳脫成 `\|`、換行變空白。
- 會把表格弄壞的地方要避開：**段首縮排**（前面多兩個全形空格就不是表格了，
  `shouldIndentLine`）、**表格行按 Enter 自動縮排**（`handleEditorEnterKey`，
  要在 `preventDefault` 之前放手）。以後加任何「逐行改內文」的功能都要想到表格。
- 寫回內文走 `onContentChange()`（存檔、字數、標籤、復原紀錄都在那條路上）。
  不用 `execCommand`：視窗關掉時焦點不在輸入框，它會插到別的地方去。
- 閱讀模式只在這一次開著時有效，不存：下次開 app 找不到輸入框會以為壞了。
- 閱讀模式裡的跳轉（章節、標籤、快速跳轉）**留在閱讀模式**，捲到那一行閃一下
  （`jumpInReadingView`）。第一版是先切回編輯，使用者回報「一跳就變回編輯模式」——
  跳過去是要看，不是要改。空行、表格分隔線沒有自己的元素，找行號不超過目標的
  最後一個。
- HTML 匯出維持 `<pre>` 純文字，這樣匯出的檔案還能原樣匯回來。
- 行內格式（`**粗體**`、`*斜體*`、`~~刪除線~~`）只在閱讀模式畫：輸入框沒辦法混排
  粗細不同的字，高亮層也會跟字錯開。符號內側緊貼的字不能是空白或符號本身。
- `## 小標題`（`###` 再小一級）與 `- 列點`／`* 列點`（前面每兩格一層）也只在閱讀模式畫。
  `##` 有列進章節快速跳轉與 📑 面板（使用者要求；`###` 以下不列）——兩邊都走
  `tocHeadingsOf()`，不要再各寫一份判斷。這兩種行跟表格一樣要避開**段首縮排**與 **Enter 自動縮排**
  （縮排會把列點變成下一層、把 `##` 變成一般段落）。
- 雙擊回到編輯：每一行（段落的一行、標題、表格的一列）掛 `data-line`，每一段
  掛 `data-o`（符號之後在原文那一行的第幾個字），`caretRangeFromPoint` 點到的
  位置加回去就是原文的游標位置。手指自己看兩次 touchend（iOS 不保證發
  dblclick），要在同一個手勢裡聚焦輸入框才叫得出鍵盤。表格「點一下開視窗」
  延後 300ms，雙擊的第二下會取消它。
...
使用者的規格（2026-09）：Supabase 是主要來源，但不可以因為雲端比較新就蓋掉這台
還沒同步的修改；兩邊都改過就是衝突，**不要自己合併、不要覆蓋任何一邊**，暫停
那一筆的自動同步，讓使用者選用這台的／用雲端的／兩份都留。選了之後才更新同步
時間。

做法（`mergeAppData()` 的 `data` 與 `upload`）：

- 合併照樣算，沒撞到的照常同步。撞到的那幾筆**這台顯示這台的（`data`）、推上去
  的是雲端原本的（`upload`）**。`applyMergedData()` 推的是 `upload`，指紋也記
  `upload`（＝雲端現在的樣子）。**推 `appData` 就等於沒問就覆蓋了雲端**。
- 撞到的記進 localStorage 的 `wb_sync_record_conflicts`（只存 kind／id／標題）。
  重開 app 也要記得。
- **祖先裡那幾筆換成一個誰都對不上的記號（`CONFLICT_SENTINEL`）再合併。**
  少了它：推完一輪之後祖先記成雲端那一版，這台的看起來就是「只有本機改」，
  下一輪會安靜地把這台的推上去。兩邊後來改成一模一樣的話，衝突自然消失。
- 有沒決定的衝突時：`pushNow()` 不可以直接推 `appData`，要先抓雲端走合併；
  對帳不可以整包採用雲端（`adoptRemote`），那幾筆這台的版本還沒上去。
- 使用者選了之後（`resolveRecordConflicts` → `applyRecordChoice`）：先抓一次雲端
  最新的，「用這台的」把那一筆的祖先記成雲端現在的樣子（下一輪就是只有本機改）、
  「用雲端的」把本機那一筆換成雲端的、「兩份都留」把這台的另存成「（衝突副本）」。
  然後再合併一次推上去——同步時間在這時才更新。
- 只有新的衝突才跳視窗；按過「稍後再決定」的，設定面板的同步那一列會寫「N 項
  衝突待決定」，同步面板裡有按鈕叫回來。

**白板逐個物件。** 世界觀拆成「本身的欄位」（`worldMetaOf`）與白板上每一個節點／
連線／便條紙，指紋多兩欄 `worldMeta` 與 `canvas`（鍵是 `世界觀id|nodes|物件id`）。
兩邊都有這個世界觀時逐個物件合併；只有一邊有（另一邊刪了）時還是看整個世界觀，
一邊刪、另一邊改了它的白板＝衝突。舊指紋沒有這兩欄時，先看是不是只有一邊動過，
兩邊都動過才逐個物件、取聯集。

白板物件的 `createdAt`／`updatedAt`（ISO 字串）只在真的建立或改了內容時蓋
（`stampCanvasCreated`／`touchCanvasObject`）。**舊資料不補**：兩台裝置各自補上
的時間不一樣，每個物件都會同時「兩邊都改過」，下次同步全部變衝突。

**對帳前先 `peek()` 版本**，跟上次同步記下的一樣就不下載整包。雲端還是整包存成
一列（使用者決定先不動資料庫），所以真的要下載時仍然是整包；上傳也是整包。
逐筆傳輸要另開逐筆的資料表，見「還沒做的」。

**使用者回報：兩台改同一塊白板的不同便條紙，一直跳衝突，另一台閒置也一樣；
按用雲端／用本機都沒反應；同一個東西同步三次。** 單元測試全綠，因為假雲端是
瞬間回應的。加上網路延遲在兩個瀏覽器裡跑才重現，是三個湊在一起：

1. **上傳在路上時又打字。** 原本上傳回來之後才拿 `appData` 記指紋、清「有未
   上傳修改」——記到的是根本沒上去的字。下一輪合併以為這台沒改，要嘛把字吃掉，
   要嘛把自己的便條紙判成衝突。改成出發前拍快照（`pushData` 的 `sent`），
   回來時 `appData` 跟出發時不一樣就保留旗標、接著再推。
2. **同時有兩個上傳。** 慢網路下 debounce 會在前一個還沒回來時再叫一次，兩個
   帶同一個版本號出去，後到的撞到自己。現在一次只推一個（`claimPush`／
   `releasePush`），期間再來的只記一個「等一下再推」。
3. **合併完祖先沒更新。** 原本合併後要等推送成功才記指紋；推送又撞到（另一台
   一直在推）的話，下一輪拿舊祖先合併，上一輪從雲端拿進來的東西變成「這台改的」
   → 另一台在打的便條紙在這台跳衝突。合併完就把祖先記成那一版雲端
   （`applyMergedDataLocally`）。

加上：連撞到 `MERGE_PUSH_MAX_RETRIES` 不再跳整包二選一的視窗（連撞不是衝突），
改成退開隨機幾秒再推。以前那個視窗跳出來之後計數不歸零，按哪一邊下一撞又跳回來，
看起來就是「按了沒反應」。以及：正在編輯便條紙時不重畫白板
（`renderCanvasWhenNotEditing`），重畫會把焦點弄丟、後面打的字不見。

**教訓：同步的測試一定要有網路延遲。** 瞬間回應的假雲端永遠不會出現「上傳途中
又改了」與「兩個上傳同時在路上」，而這兩個正是實際使用最常見的狀況。
`tests/sync-e2e.test.js` 的假雲端現在每次推送都會先等一下，並且可以在途中
插入另一台的推送（`beforePush`）。

**閒置 3 分鐘自動下載一次**（`shouldIdleSync`）：看的是距離最後一次操作多久
（`lastActivityAt`，閒置備份提醒那邊在記），同一段閒置只做一次。原本那個固定
每 3 分鐘掃一次的計時器拿掉了；切回前景、換文檔等時機照舊。
...
所有世界觀共用一個垃圾桶，照世界觀分組、每組標上世界觀的名稱；刪掉的世界觀
放最上面（最近刪的在前），接著是現有的世界觀、照左側那排的順序。「清空」＝全部。

（中間有一版是「每個世界觀各自一個垃圾桶」，使用者用過之後改回來了——
「全部放在一起，標示好是哪個世界觀的就好」。不要再改回分開。）

**刪掉的世界觀整個復原**（使用者：「復原直接復原整個世界觀，不需要把裡面有什麼
列出來」）。垃圾桶裡一個刪掉的世界觀就是一列，按「復原」整個回來。

- `deleteWorldById()` 在動任何東西之前，把整筆（名稱、圖示、簡介、**整張白板**）
  存進 `trash.worlds`。以前這一筆是直接丟掉的：白板上的便條紙與連線說明刪了就
  永遠回不來，沒有文檔的世界觀刪掉之後什麼都不留。
- 隨它一起進垃圾桶的文檔／資料夾標 `trashedWithWorld`。**復原只帶這些回去**——
  在那之前就個別刪掉的不該跟著復活，那是使用者本來就丟掉的。
- 白板節點已經在整筆紀錄裡了，**不要再拆一份進垃圾桶**（`moveDocsToTrash` 的
  `keepCanvas`），不然復原時會變成兩份。
- 名字也還是蓋在每一筆上（`stampDeletedWorldOnTrash()`）：那是「更早就刪掉、
  後來世界觀也沒了」的項目認得出主人的唯一線索，也是舊資料（刪的時候還沒有
  `trash.worlds`）重建世界觀時唯一拿得到名字的地方。舊資料沒蓋過名字的，寫
  「名稱沒有留下來的世界觀」。
- `trash.worlds` 是 trash 底下新加的欄位，**合併、60 天清除、匯入備份、清空**
  四個地方都有管到（見 3m）。

白板項目是用「在原本陣列裡的索引」復原與刪除的——**分組前先把索引記下來**，
改用組內的索引的話，按「復原」會復原到別的那一筆。

**因為刪文檔而進來的節點跟著文檔走**（使用者：「不用獨立出來」）。
`trashOrphanNodesForDocs()` 拆節點時標 `withDoc`（舊資料靠「隨文檔一起刪除」那個
固定標籤認，見 `canvasTrashFollowsDoc()`）。那篇文檔還在垃圾桶時這些節點不列出；
復原文檔時一起回白板、永久刪除文檔時一起刪。幾個細節：

- **跟著復原要從後往前做**：每復原一筆就從陣列拿掉一筆，從前往後的話，相鄰的
  第二筆會遞補到剛剛的位置然後被跳過。
- **跟著復原失敗不跳提示**（`restoreCanvasTrashItem(i, true)`）：使用者按的是
  「復原文檔」、文檔也確實回來了，跳一個關於節點的警告會讓人以為文檔沒成功。
  放不回去的留在垃圾桶，那時文檔已經不在垃圾桶了，它就會變成獨立一列、看得到。
- **文檔搬到別的世界觀時留下的節點不標**：那篇沒被刪。標了的話，它之後在新世界觀
  被刪再復原時，會把舊世界觀的節點帶回舊白板。

### 3m. 合併的時候，每一種資料都要有人管

`mergeAppData()` 以前組 `trash` 時只放了 `docs` 與 `folders`，**`trash.canvas`
（白板刪掉的節點／連線／便條紙）在任何一次自動合併之後都會整個消失**——而兩台
各改一篇就會觸發自動合併。它是後來才加的，合併這裡沒跟著改。已修：

- 那些項目本身沒有 id，用 `canvasTrashKey()`＝「種類＋內容 id＋刪除時間戳」當鍵，
  包成 `{ id, entry }` 之後走跟文檔一樣的三方合併。**時間戳一定要有**：節點 id 是
  `"node_" + 文檔 id`，投射、刪掉、再投射、再刪掉，兩次的 id 一樣。
- 指紋多一欄 `trashCanvas`。升級前的舊指紋沒有它：分不出「刪了」還是「從來沒有」，
  退成兩邊的聯集——**寧可多留一筆也不要丟**。
- 組不出鍵的（缺欄位）照本機的留著。
- `trash` 底下以後多出來的欄位照本機的留著（跟頂層同一條規則）。

`trash.worlds`（整筆刪掉的世界觀）也一樣接進來了：它有 id，直接走一般的三方合併。
「照本機的留著」在這裡不夠——那只保得住這一台刪的，另一台刪的世界觀會永遠看不到。

**以後在 `appData` 或 `appData.trash` 底下加任何新欄位，都要回來看四個地方：
合併（`sync-merge.js`）、60 天清除（`storage.js`）、匯入備份（`import-export.js`）、
清空垃圾桶。** 沒管到的後果不是報錯，是某一次同步或還原之後它安靜地消失。
...
```bash
node --test          # 210 項。注意：不要寫 node --test tests/，Node 22 會去 require 那個目錄
```
...
| 刪掉已合併的舊分支 | 使用者說「等等再刪」 |
| 逐筆傳輸（只傳改過的那幾筆） | 要在 Supabase 另開逐筆的資料表、使用者要重跑 SQL。使用者說「先不要動資料庫」。目前做到：雲端沒變就不下載（先問版本）、內容沒變就不上傳 |
