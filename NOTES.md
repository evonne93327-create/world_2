...
**刪掉的世界觀的名字**：世界觀刪掉之後名字就不在任何地方了，所以
`deleteWorldById()` 在拿掉它**之前**呼叫 `stampDeletedWorldOnTrash()`，把名字蓋在
垃圾桶裡屬於它的每一筆上（含之前就從它刪進來的）。**不要改成另外存一份「已刪除
的世界觀」清單**：trash 底下的欄位要合併那邊有處理才會留下來（見 3m），蓋在每一筆
上它就跟著那一筆被合併。更早以前刪的世界觀沒蓋過名字，寫「名稱沒有留下來的世界觀」。
...
**以後在 `appData` 或 `appData.trash` 底下加任何新欄位，都要回來看合併有沒有
管到它。** 沒管到的後果不是報錯，是某一次同步之後它安靜地消失。
...
```bash
node --test          # 185 項。注意：不要寫 node --test tests/，Node 22 會去 require 那個目錄
```
